...
export const lyzrProvider: AIProvider = {
  ...
  async poll(task_id: string) {
    ...
    const task = (await res.json()) as { status: string; response?: unknown; error?: string }
    ...
    const toNorm = parsed && typeof parsed === 'object' && (parsed as Record<string, unknown>).success === false && (parsed as Record<string, unknown>).data === null
      ? agentResponseRaw
      : parsed
    const normalized = normalizeResponse(toNorm)
    return { status: 'completed' as const, response: normalized }
  },
}
