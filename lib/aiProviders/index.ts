...
export function getAIProvider(): AIProvider {
  ...
  if (!provider.isConfigured()) {
    throw new Error(
      `AI_PROVIDER="${id}" is not configured. Set the required env vars (e.g. ${id === 'lyzr' ? 'LYZR_API_KEY' : id === 'openai' ? 'OPENAI_API_KEY' : id === 'anthropic' ? 'ANTHROPIC_API_KEY' : 'GOOGLE_AI_API_KEY'}).`
    )
  }
  return provider
}
