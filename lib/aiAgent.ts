...
export async function callAIAgent(
  ...
  try {
    ...
    // Sync response (OpenAI, Anthropic, Google): return immediately
    if (submitData.status === 'completed' && submitData.response) {
      return {
        ...submitData,
        agent_id,
        success: submitData.success !== false,
      }
    }

    // If submit failed, return as-is
    if (!submitData.task_id) {
      return submitData.success === false
        ? submitData
        : {
            success: false,
            response: { status: 'error', result: {}, message: 'No task_id or completed response' },
            error: 'No task_id or completed response',
          }
    }

    const { task_id, user_id, session_id } = submitData

    // 2. Poll POST /api/agent with { task_id } — adaptive backoff (Lyzr only)
